...
export const Compras = () => {
  ...
  useEffect(() => {
    const fetchProveedores = async () => {
      ...
      for (const compra of compras) {
        if (compra && compra.proveedor && typeof compra.proveedor === 'string') {
          ...
          if (!proveedoresCache[compra.proveedor]) {
            try {
              ...
              console.error(`Error al obtener proveedor ${compra.proveedor}:`, error);
            }
          }
        }
      }
  ...
  const filteredCompras = useMemo(() => {
    return compras.filter((compra) => {
      ...
      const proveedorObj = compra.proveedor && typeof compra.proveedor === 'string' 
        ? proveedores[compra.proveedor] 
        : (typeof compra.proveedor === 'object' ? compra.proveedor : null);

      // Filtrar por búsqueda (nombre del proveedor)
      const proveedorPorNombre = searchTerm && proveedorObj && proveedorObj.nombre
      ...
      const estadoSeleccionado = selectedStatus ? selectedStatus.toLowerCase() : "";
      
      // Si hay un estado seleccionado, filtrar por él, de lo contrario incluir todas
      const filtradoEstado = estadoSeleccionado
        ? estadoCompra.includes(estadoSeleccionado) || estadoSeleccionado.includes(estadoCompra)
        : true;
      ...
      <Card>
        ...
        <div className="flex justify-between items-end mb-4">
          ...
          <HeaderProcess
            ...
            statusOptions={["Pendiente", "Procesando", "Completado", "Cancelado"]}
          />
        </CardHeader>
        <CardContent>
          <ComprasTable
