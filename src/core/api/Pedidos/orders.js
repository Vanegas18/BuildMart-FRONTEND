...
export const createOrder = (pedidos) => axios.post("pedidos", pedidos);
...
export const updateOrderStatus = async (pedidoId, nuevoEstado) => {
  try {
    ...
    const response = await axios.put(`pedidos/${pedidoId}`, { estado: nuevoEstado });
    return response.data;
  } catch (error) {
    console.error("Error al cambiar el estado del pedido:", error.response || error);
    throw error;
  }
};
